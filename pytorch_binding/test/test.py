...
def small_test():
    B = 1; T = 4; U = 3; V = 5
    trans_acts = torch.tensor([[[0.06860042, 0.02114975, 0.9319364,  0.32975072, 0.13327402],
    ...
    pred_acts = torch.tensor([[[0.1928187,  0.63991195, 0.7839058,  0.8364832,  0.55822134],
    ...
    label = torch.IntTensor([[1, 2]])

    act_length = torch.IntTensor([T] * B)
    label_length = torch.IntTensor([U-1] * B)
...
def big_test():
    ...
    trans_acts = torch.Tensor([0.20836472511291504 ,0.6848891377449036 ,0.8508703112602234 ,0.5761988759040833 ,0.19992691278457642 ,0.8066366910934448 ,
