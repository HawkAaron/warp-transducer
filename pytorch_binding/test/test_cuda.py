...
def generate_sample():
    B = 1; T = 4; U = 3; V = 5
    trans_acts = torch.zeros(B, T, V, device=torch.device('cuda'), requires_grad=True).uniform_()
    pred_acts = torch.zeros(B, U, V, device=torch.device('cuda'), requires_grad=True).uniform_()

    label = torch.zeros(B, U-1).uniform_(1, V-1).int()
...
def small_test():
    ...
    assert np.allclose(loss.item(), expected_cost), \
        "costs mismath."
    assert np.allclose(trans_acts.grad.cpu().numpy(), expected_trans_grads, rtol=1e-4), \
        "trans gradient mismatch."
    assert np.allclose(pred_acts.grad.cpu().numpy(), expected_pred_grads, rtol=1e-4), \
...
def big_test():
    ...
    trans_acts = torch.tensor([0.20836472511291504 ,0.6848891377449036 ,0.8508703112602234 ,0.5761988759040833 ,0.19992691278457642 ,0.8066366910934448 ,
                                ...
                                , device=torch.device('cuda')).view(T, B, V).transpose(0, 1).contiguous().requires_grad_(True)
    ...
    pred_acts = torch.tensor([0.06116640567779541 ,0.14563453197479248 ,0.5638840198516846 ,0.6632290482521057 ,0.19838422536849976 ,0.1820780634880066 ,
                                ...
                                , device=torch.device('cuda')).view(U, B, V).transpose(0, 1).contiguous().requires_grad_(True)

    label = torch.IntTensor([[1, 2], [1, 1]])
    ...
    assert np.allclose(loss.item(), expected_cost), \
        "costs mismath."
    assert np.allclose(trans_acts.grad.cpu().numpy(), expected_trans_grads), \
        "trans gradient mismatch."
    assert np.allclose(pred_acts.grad.cpu().numpy(), expected_pred_grads), \
        "pred gradient mismathc."
